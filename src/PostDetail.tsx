  ...
  const handleBookmark = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/posts/${post?._id}/toggle-bookmark`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId }),
  ...
  return (
    <div className="detail-container">
      ...
      <div className="detail-blocks-container">
        {post.blocks?.slice(0, visibleBlocksCount).map((block, idx) => (
          <p key={block.id || idx} className="detail-block">
            {block.data?.text || ""}
          </p>
        ))}
      </div>
      ...
      {/* Load More Button */}
      {post.blocks && visibleBlocksCount < post.blocks.length && (
        <button
          className="load-more-button"
          onClick={() =>
            setVisibleBlocksCount(
...
function CommentCard({
  ...
  const handleEdit = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/posts/${postId}/comments/${comment._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
